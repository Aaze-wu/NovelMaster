...
- � **检查更新**: 「帮助 → 检查更新…」看一眼 GitHub 上的最新正式版；也可以在设置里
  打开「启动时检查更新」（**默认关闭**，且一天只查一次），发现新版本时下载安装包、
  确认后启动安装程序并退出主程序。便携版（zip 解压运行的）不自动安装，只引导你去
  发布页下载；GitHub 直连慢可在设置里填一个下载镜像前缀（内置 `gh-proxy.com` 兜底）
...
| `pypinyin>=0.55.0` | 朗读的**多音字读音纠正**（v1.4.2）：只为“自动推断多音字”那一层提供注音（MIT） | 内置规则与自定义词典照旧可用，只是多一个开关 |
...
- **多音字读错了**：“朗读” → “读音纠正（多音字）…”，或**在正文里选中读错的那个词点右键**，
  见下文“多音字读音纠正”；
...
| 从光标处开始 | 从光标所在**那一句的句首**读起，读到本章末尾，然后**接着往下读下一章 / 下一篇**（听“读完自动读下一章”开关） |
...
“从光标处开始”读到章末算不算“读完”则交给**“读完自动读下一章”开关**：勾着就接着读
下一章（新章从章头整章读），取消勾选就停在章末；定着“读完本章停”同样停在章末。
“只读选中内容”永远是一次性的。
...
朗读引擎（包括在线引擎）按词猜多音字，猜错就读错：“银行”读成 `yín xíng`、“重新”读成
`chóng xīn` 还是 `zhòng xīn` 全看运气。**朗读引擎这边的语音合成（SSML）接口用不上**
（实测在线接口只接受纯文本），所以本项目的做法是**只送给引擎一份“改了字”的文本**：
把读错的字换成**读音相同、而且只有这一个读音**的常用字，例如把 `银行` 换成 `银航`，
引擎就不会再猜错。

- **正文、逐句高亮、自动滚动全都不变**：换字只发生在送进引擎前的那一瞬间，
  屏幕上的文字、书签、阅读进度、复制粘贴都是原文；
- 窗口**不是模态**的，改完**立即生效**（不用点“确定”），也能一边读一边改；
- 菜单里有两个入口：“朗读” → “读音纠正（多音字）”与“设置” → “读音纠正（多音字）”，
  用的是同一个窗口；
- **正文里选中一个词，右键 →“把选中的字加入读音纠正…”** 就能直接填进“原词”框，
  已经是词典里的词（内置或自定义）还会把“改成”一起带出来，改一个字点“保存”即可。
  选中的文字里的空白与标点会被自动滤掉（`千绝地。`、`「千绝地」` 都认成 `千绝地`），
  选中超过 6 个字会在窗口下方提醒一句“多半多选了”。

> **生造的地名（`千绝地` 这种）只能靠这条路一条条加。** 引擎和 `pypinyin` 都会把
> “地名末尾的『地』”按它最常见的读音 `dì` 读，而这类词该读轻声 `de`；
> **没有任何上下文线索能把这两种情况分开**——实测 2M 字语料，“地”出现 5389 次，
> 其中后面跟标点的 281 次里约 **98% 是实词**（土地 / 目的地 / 原地 / 领地 / 各地…）
> 该读 `dì`，一律改成 `de` 会误伤“很快地，”“相对地，”这类助词。
> 词典条目短（2~4 字）反而更可靠，所以把“千绝地”整个词加进来最稳。

**三层来源**（从高到低，同一条词以更靠前的那层为准）：

| 层 | 内容 | 怎么用 |
| --- | --- | --- |
| 自定义 | 你自己加的词，例如 `银行 → 银航` | 窗口中间那两个输入框，填“原词 / 改成”后点“保存” |
| 内置 | 56 条常见错读（银行 / 重新 / 音乐 / 单薄 / 给予 / 系鞋带…），列在表里，带“说明”一列写清楚该读什么 | 默认开，可删除、可“恢复默认” |
| 自动 | 用 `pypinyin` 按上下文猜多音字，只在**猜出的读音和默认读音不一样**且**词表里已确认过该读音**时才动手 | **默认关**，装了 `pypinyin` 才能勾 |

窗口上方两个开关：**启用读音纠正**（总开关，关掉 = 完全按原文送引擎）与
**自动推断多音字**（默认关）。两个开关与自定义词都存在
`%APPDATA%\NovelMaster\pronunciation.json`（**不写 `config.json`**），改一下存一下。

- 表格与输入框下方有一行**预览**，拿一句“他去银行取钱，重新数了一遍，又买了张薄饼。”
  实时显示“实际送给引擎的文本”，改词的时候看一眼就知道对不对；
- 删除**内置**的词只是把它标成“已停用”（下次不会替你改字），不会真的删掉，
  想回到出厂状态点“恢复默认”即可；
- 自定义词填“改成”那一栏留空 = **不替换**（用来把某条内置规则按下去）；
- 替换字一律挑**单一读音**的字（例如 `己` 有 `jǐ` / `qǐ` 两读，就换成只有 `jǐ` 的 `挤`），
  否则等于把一个多音字换成另一个多音字。

> **为什么“自动推断”默认关着？** 实测拿 503 段、5158 字的语料跑一遍，自动层提出 46 处改动，
> 几乎全是废话（`一 → 宜`、`行 → 航`、`不 → 醭`…）—— 也就是说 `pypinyin` 的上下文模型
> 和引擎自己的判断**高度重合**，引擎错的地方它多半也错。加上“词表确认”这道闸门后这 46 处
> 归零，所以默认关着当**安全网**用；真遇到个别漏网的，左边自己加一条更省事。
...
- `config.json`: 程序配置（含 `language` 界面语言、`shortcuts` 键盘快捷键绑定、
  ...
  `auto_check_update`（启动时检查更新，默认关闭）、`update_mirror`（更新下载镜像
  前缀，默认空）与 `update_last_check`（上次自动检查的时间戳，用来保证一天最多查一次）
  （朗读的**定时停止档位**只在内存里，不会写进配置文件）
- `update/`: 已下载的更新安装包（下完会等你确认再装；取消下载会留下 `.part`，
  下次接着下）
...
本版新增**检查更新**：程序自己去 GitHub 看有没有新版本，下载安装包，确认后启动
安装向导并退出主程序 —— 用户要做的只有点两下按钮，不用再翻发布页找文件。

- **入口两处，互不打扰**：「帮助 → 检查更新…」随时可点；「设置 → 启动时检查更新」
  是**可选项，默认关闭**（不打招呼就联网问 GitHub 不合适），打开后**一天最多查一次**
  （`update_last_check` 记时间戳），查出新版本才会弹一句「v1.4.4 已经发布，现在看看吗？」。
- **界面只有一屏，状态自己切**（`enm/ui/update_dialog.py`）：检查中 → 发现新版本 →
  下载中 → 安装包就绪，另外两个终点是「已是最新」和「检查失败（可重试）」；
  更新说明按 Markdown 渲染（限高 260 px，再长不撑爆窗口）。
- **便携版不给自动装**：安装程序装出来是 `Program Files` 下的**另一份**，跟便携版
  并排躺着，用户只会以为更新没生效。所以便携版（zip 解压运行）这一路把主按钮换成
  「前往下载页」，由用户自己去拿 `portable.zip`；判断依据是程序目录下有没有
  Inno Setup 留下的 `unins*.exe`（`update.is_installed_copy()`），**源码运行也按便携版处理**。
- **装之前再问一次**：就算下载完了也不擅自退出 —— 退出是不可逆的（正在朗读、
  刚翻到一半的章节都在那儿结束）。对话框只把安装包路径记在 `install_path` 里，
  `exec_()` 返回后由主窗口 `quit_and_install()` 启动安装程序（带 `/CLOSEAPPLICATIONS`
  兜底）再退出；启动失败会在窗口里提示手动运行。
- **退出时机放在主窗口**：不在对话框回调里退出，避免「信号发完窗口已经没了」这类
  顺序问题；托盘也不会把这次退出当成「关闭窗口藏到托盘」（`_quitting_for_update`）。
- **下载能续、能取消、能换镜像**（`enm/managers/update.py`，写法照搬朗读模型那套）：
  256 KB 一块、HTTP 206 断点续传、失败重试 3 次；直连失败自动换内置镜像
  （`gh-proxy.com`），设置里还能再填一个自定义前缀（排在最后，留空即只用内置）；
  取消后 `.part` 留着，下次接着下；已经下完的安装包直接复用。
- **只认正式版**：`releases/latest` 只返回正式版，代码里再确认一遍不是
  draft / prerelease，避免把预发布当更新推出去。
- **失败都能读懂**：404（还没发过正式版）、403（匿名接口额度用完，同一出口 IP
  每小时 60 次）、断网/超时、下载中断，各有各的中文提示，不会甩一串英文异常。
- **顺带修了两处小毛病**：`update.available_detail` 原本写成
  「当前 v{version} → 最新 v{version}」，两个占位符同名会把当前版本也填成新版本号，
  现在是 `{current}` / `{version}` 两个不同的值；「朗读 → 选择音色…」在没有可用
  语音引擎时取的键名写成了 `menu.tts_unavailable`（实际是 `msg.tts_unavailable`），
  通知条上会直接显示这串英文键名。
- **无新增必需依赖**：HTTP 走标准库；`lang/` 三个语言文件同步补上 3 个菜单键与
  一整个 `update` 块（42 键），共 598 个键，三份对齐。

### v1.4.3

本版把**文件夹模式**（一本书 = 一堆按章节拆好的 txt）下几个一直别扭的地方理顺了，
外加读音纠正的一个便捷入口和几处与实际运行环境相关的修复。

- **章节划分：不再给每个 txt 凭空加一个「序章」**
  - 现象：按章节抓成一堆 txt 的小说（每个文件开头有一行站点广告 / 页眉，真标题
    在它下面），章节列表里每个文件都多出一个只含广告的「序章」。
  - 原因：`TxtReader.split_chapters()` 把**第一个标题行之前**的内容一律单独切成
    一章，并兜底命名为「序章」。
  - 现在：标题之前的内容**短于 200 字**（`PROLOGUE_MIN_CHARS`，广告 / 页眉这类）
    直接并进后面那一章；超过 200 字（真正的序文）才保留为独立的序章。
  - 实测：《元龙》文件夹 1566 个 txt，以前 **1418 个（91%）**都带「序章」，
    现在 0 个；`0184 …聪明人（下）.txt` 从 2 章变回 1 章。
  - 注：并进正文的那几行会作为该章的第一段显示，也仍会被读出来；想连广告行一起
    丢掉的话需要再加一条「跳过抓取站广告行」的规则。
- **「从光标处开始朗读」读完本章会接着往下读（行为变更）**
  - 现象：右键「从光标处开始朗读」读到章末就整体停了 —— 它以前是纯一次性范围，
    只有「指定起止章节」才会接力，所以一直用它起读的人会觉得「文件夹模式不能自动
    读下一章」（日志里就是 `kind: 'cursor'` 后面跟着 `朗读范围读完，停止朗读`）。
  - 现在：光标范围读到章末，**勾着「自动读下一章」就接着读下一章 / 下一篇**
    （范围本身作废，新章从头整章读）；取消勾选就停在章末；定着「读完本章停」也停在
    章末。「只读选中内容」仍然是一次性的（用户点明了只读这一段）。
  - 另外，章节划分修好后，翻篇不再先落进那个「序章」广告章 ——
    `next_chapter()` 的跨文件逻辑自 v1.3.11 起就是好的，问题只在这个假章上。
  - 实测（假引擎按真实时序异步回报「读完」、跑完整事件循环）：在
    `0189 第一百二十五章 出城（上）.txt` 章末起读（只切出 5 句）→ 自动翻到
    `0190 第一百二十五章 出城（下）.txt` 并继续朗读，`state=playing`；
    关掉开关、以及定着「读完本章停」两种情况都在章末如期停下。
- **阅读进度：文件夹模式改按整本书算**
  - 以前按**当前文件**的章节数算，所以每翻到一个文件的第 2 章就显示 `100%`。
  - 现在 `(已读完的文件数 + 当前文件内的章位置) / 文件总数`：《元龙》读到第 184 篇
    显示 `11.7%`，最后一篇 `100.0%`。
- **打包（`build.ps1`）：补上 chardet 的数据文件**
  - chardet 7.x 把统计模型放在 `chardet/models/*.bin`，只写
    `--include-package=chardet` 会收 `.py` 但不收这些 `.bin`，冻结版里
    `chardet.detect()` 直接 `FileNotFoundError`（日志里刷
    `检测编码失败: … models\models.bin`），TXT 只能退回 `utf-8 + errors='ignore'`
    —— 实测一本 GBK 小说这样解**会丢掉一半字符、剩下的全是乱码**，也就是
    「打包版打开 GBK 小说整篇乱码」。
  - 现在加上 `--include-package-data=chardet`，与 `pypinyin` / `winrt` /
    `sherpa_onnx` 的写法一致。
- **正文右键「把选中的字加入读音纠正…」**：正文里选中一个词，右键就能把词直接填进
  读音纠正对话框的「原词」（这个词已经在表里的话，连带把「改成」「说明」带出来，
  改一个字就是一条覆盖记录）；选中的内容超过 6 个字（`PREFILL_MAX_LEN`）会在状态栏
  提醒一句「多半把上下文一起选进来了」。
- **`Pronouncer` 接受字符串路径**：`__init__` 里补一次 `Path(path)` 转换，
  传字符串路径时不再抛 `'str' object has no attribute 'exists'`
  （`load()` / `save()` 直接按 `Path` 用 `self.path`）。

### v1.4.2

本版新增**多音字读音纠正**：朗读引擎（包括在线引擎）分不清多音字，`银行`、`重新`、`音乐`、
`单薄` 这类词读错得很稳定。本版给朗读加一层“换字”前置处理 —— **只送给引擎一份改了字的
文本**，屏幕上的正文、逐句高亮、阅读进度一个字都不改。

- **三层来源**（同一条词以更靠前的那层为准，`enm/managers/tts_pron.py`）：
  - **自定义词典**（窗口里自己加的词）；
  - **56 条内置规则**（`BUILTIN_RULES`）：银行 / 重新 / 音乐 / 单薄 / 给予 / 系鞋带 等
    常见错读，可以用“恢复默认”一键还原；
  - **自动推断**（`pypinyin`，**默认关**）：按上下文猜读音，且只在“猜出的读音 ≠ 默认读音”、
    且“词表里确实确认过这个读音”时才替换。
- **设置窗口** `enm/ui/tts_pron_dialog.py`：非模态、改完立即生效，带**预览**——实时显示
  “实际送给引擎的文本”；“朗读”与“设置”两个菜单都能打开它。
- **内置规则全部可编辑**：改词、停用内置词、删除、加自己的词；“改成”留空 = 不替换（用来
  压掉某条内置规则）。
- **替换字坚持选单一读音的字**：例如 `己` 有 `jǐ` / `qǐ` 两读，就换成只有 `jǐ` 的 `挤`，
  不会把一个多音字换成另一个多音字。
- **总开关**关掉 = 完全按原文送引擎；**两个开关与自定义词典**都存在
  `%APPDATA%\NovelMaster\pronunciation.json`（不写 `config.json`），改一下存一下。
- **无新增必需依赖**：内置规则与自定义词典是纯数据 + 标准库；只有“自动推断多音字”
  那一层需要可选的 `pypinyin`（缺了那个开关直接置灰）。
- **顺手修好一个既有小毛病**：音色 / 音色管理 / 读音纠正这三个窗口**开着的时候换主题**
  现在会立刻跟着变（以前要关掉重开），窗口里的原生标题栏颜色也一起更新。

### v1.4.1

本版修好**文件夹模式下章节名显示「全文」**的问题：整篇只有一章的文件（没有章节标记的
TXT、单章的 HTML / DOCX / FB2 / MOBI / UMD，以及只有一段正文的 `.jar` / `.zip`），
章节名以前是占位词，现在显示**去掉扩展名的文件名**。

- **判定规则**（`enm/readers/folder.py` 新增 `display_name()` / `_use_file_name_as_title()`）：
  只在这个文件**整篇只有一章**、且这一章的标题为空或是占位词（`全文` / `正文` /
  `Full Text` / `body` / `序章` 等，复用 `enm/managers/book_identity.py` 的
  `is_generic_title()`）时才换成文件名；**文件里真实存在的章节名一律优先**，
  多章文件完全不受影响
- **换名时同时丢掉 `title_spec`**，所以切换界面语言不会再把「全文」生成回来
  （`retranslate_titles()` 会对已缓存的每个文件重新走一遍这套判定）
- **只改了阅读器层**：章节树、正文页标题、媒体面板、窗口标题本来就都取
  `get_chapter_title()`，因此无需改动 `enm/ui/main_window.py`
- **保持不变**：上层文件节点仍显示带扩展名的完整文件名，窗口标题与书签记录也不变；
  没有新增依赖，配置字段无变化，可直接覆盖安装
