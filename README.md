...
- 🔊 **朗读（TTS）**: 用系统语音把当前章节读出来，逐句高亮 + 自动滚动、语速五档、
  ...
  （只读选中 / 从光标处 / 指定起止章节），读完自动接着读下一章；走系统自带语音引擎，
  **离线、不必联网**，装了 `pywin32` 还能多看到 OneCore 语音库里的 Kangkang / Yaoyao
...
用系统语音把书籍读出来。Windows 上走系统自带的 SAPI5 语音引擎，**完全离线**，
不需要联网。引擎有两条路，程序自己挑：

- **`sapi-com`**（优先）：装了 `pywin32` 时直接驱动 SAPI 的 `SpVoice`。好处是能看到
  **OneCore 语音库**（`HKLM\SOFTWARE\Microsoft\Speech_OneCore\Voices`），
  中文多出 Kangkang（男声）/ Yaoyao（女声）；
- **`sapi`**（回落）：没装 `pywin32` 时用 `PyQt5.QtTextToSpeech`，只列得出经典
  语音库（本机通常只有 Huihui 一个中文音色）。这条路上功能一样齐全，不用装任何东西。
...
- **换语音**：“朗读” → “朗读语音”，列出引擎能看到的所有语音，名字后面会带上性别与语种
  （本机 HuiHui 路线下例：`Huihui 女声 (zh_CN)`、`David 男声 (en_US)`、`Zira 女声 (en_US)`、
...
| `tts_volume` | `1.0` | 音量 |
| `tts_voice_name` | `""` | 上次用的语音 id（形如 `zh_CN|Microsoft Huihui Desktop`，竖线分隔） |
...
│   ├── readers/              # 各格式阅读器与工厂
│   └── ui/                   # 主窗口、朗读条与朗读范围对话框、继续阅读面板、快捷键设置、
│                             # 主题编辑/管理对话框与共享样式表
