...
- 📚 **多格式支持**: EPUB、TXT、PDF、MOBI、AZW、AZW3、PRC、UMD、DOCX、FB2、HTML/HTM/XHTML、JAR、JAD、ZIP
- 📱 **手机 Java 电子书**: 直接打开 `.jar`（JavaBook / JIEQI CMS 制作），
  按内置目录还原章节与标题，正文编码自适应（UTF-16LE / BE、UTF-8、GBK…）；
  附带的 `.jad` 描述文件也能直接打开，自动定位同目录的 JAR
- 📦 **压缩包阅读**: 直接打开 ZIP（以及 ZIP 外皮的 JAR），自动挑选其中的电子书文件
...
| `.jar` | 手机 Java 电子书 | 内置 JavaBook 解析器（纯标准库）；识别不出再按压缩包解包 |
| `.jad` | MIDlet 描述文件 | 读 `MIDlet-Jar-URL` 后打开同目录的 JAR |
| `.zip` | 压缩包 | 自动解包并读取内部电子书 |
...
`enm/readers/jarbook.py` 用纯标准库解析 JavaBook / JIEQI CMS 制作的手机电子书。
这类 `.jar` 本身就是个 MIDlet，书的内容塞在里面：

- **目录**：包内名为 `0` 的条目是一串 Java `writeUTF`
  （2 字节大端长度 + UTF-8），依次是 `书名`、`章节数`，
  接着每章一条 `序号,字节数,标题`；章节正文的条目名就是这个**序号**
  （所以条目**没有扩展名**，旧版一律当压缩包处理时挑不出候选文件，
  只会报「压缩包中没有找到可阅读的内容」）；末尾还会有一段制作信息，
  作者从里面的 `作者：…` 行取
- **正文编码**：先数偶数位 / 奇数位字节上的 `0x00`，据此判 UTF-16LE / BE
  （这类文件通常不带 BOM），都不是就交给 `decode_bytes()`
  依次试 UTF-8 / GB18030 / Big5 / chardet
- **标题**：正文首行写作 `<第一卷 序章>`，解析时把它摘出来当章节标题，
  正文里不再重复；没有这行就退回目录里的标题
- **顺序**：以目录为准，包内条目顺序不可靠；目录与条目名对不上时退回数字序
- **插图页**：目录里有些条目只有一行标题、正文为空（如「第十七卷 插图」），
  按 `BaseReader` 的既有约定（`_add_chapter` 跳过空正文）不会出现在章节列表里

`.jar` 的入口是 `JarReader`（见 `enm/readers/archive.py`）：先
`JarBookReader.detect()` 判断像不像手机电子书，命中才按电子书读，
失败或一章都没读出来就记条日志退回压缩包逻辑，**ZIP 外皮的 JAR 行为不变**；
只有纯应用（包内全是 `.class`）的 JAR 仍报原来的「压缩包中没有找到可阅读的内容」。

`.jad` 只是 MIDlet 的描述文件（Java 属性文件格式，支持 `\` 续行），
正文在它指向的 `.jar` 里：`JadReader` 按 `MIDlet-Jar-URL` 找同目录的 JAR，
找不到时再看有没有「同名 `.jar`」（`X.jad` ↔ `X.jar`）—— 描述文件里写的
常常还是下载站的原始文件名（如 `2441.jar`），本地文件早被改成书名了；
指向 `http(s)://` 则直接提示无法离线阅读。打开 `.jad` 时窗口标题与
最近文件记录仍沿用 `.jad` 自己的路径。
...
本版新增**手机 Java 电子书（`.jar` / `.jad`）**支持。以前 `.jar` 一律当压缩包解包，
而这类电子书的正文条目是**纯数字名、没有扩展名**的，挑不出候选文件，
只能报「压缩包中没有找到可阅读的内容」；现在按 JavaBook 格式解析。

- **新增手机电子书解析器** `enm/readers/jarbook.py`（纯标准库）：
  - **目录**：包内名为 `0` 的条目是一串 Java `writeUTF`（2 字节大端长度 + UTF-8），
    依次是书名、章节数，接着每章一条 `序号,字节数,标题`；正文条目名就是这个序号
  - **正文编码**：先数偶数位 / 奇数位字节上的 `0x00` 判 UTF-16LE / BE
    （这类文件通常不带 BOM），否则交给 `decode_bytes()` 试 UTF-8 / GB18030 / Big5 / chardet
  - **标题与作者**：章节标题取正文首行的 `<第一卷 序章>`（并把它从正文里摘掉），
    没有则退回目录标题；书名取 `MIDlet-Name`，作者从目录末尾的制作信息里取
    （`作者：…`，制作信息是 `\r\n` 分隔的，匹配前先归一化换行）
  - **顺序**：以目录为准，包内条目顺序不可靠（目录与条目名对不上时退回数字序）
  - **插图页**：目录里正文为空的条目（如「第十七卷 插图」，只有一个标题行）
    按 `BaseReader` 的既有约定跳过，所以样本的 202 条目录最终是 185 章
- **`.jar` 入口**（`enm/readers/archive.py` 的 `JarReader.load()`）：先
  `JarBookReader.detect()`（目录条目能解析 / 清单里有 `JavaBook`、`JIEQI` /
  有清单且带一批纯数字条目），命中才按电子书读；解析失败或一章都没读出来
  就记一条日志退回压缩包逻辑，**ZIP 外皮的 JAR 行为不变**，
  只有纯应用（包内全是 `.class`）的 JAR 仍报原来的「压缩包中没有找到可阅读的内容」
- **新增 `.jad` 入口** `JadReader`：`.jad` 只是 MIDlet 描述文件（属性文件格式，
  支持 `\` 续行），正文在它指向的 JAR 里 —— 按 `MIDlet-Jar-URL` 找同目录的 JAR，
  找不到时再看有没有「同名 `.jar`」（`X.jad` ↔ `X.jar`）；描述文件里写的
  常常还是下载站的原始文件名（实测样本里是 `2441.jar`），本地文件早被改成书名，
  所以这层兜底是必需的；指向 `http(s)://` 则明确提示无法离线阅读。
  打开 `.jad` 时窗口标题与最近文件记录仍沿用 `.jad` 自己的路径
- **登记与界面**（`enm/readers/factory.py`、`folder.py`、`enm/managers/book_identity.py`）：
  `.jad` 登记进 `EXTENSION_READERS`，`.jar` / `.jad` 一起归到「电子书文件」分组
  （压缩包组只剩 `.zip`）；文件夹模式扫描时同目录已有同名 `.jar` 就不再重复
  收录 `.jad`；书名归一化同样剥离 `.jad` 后缀
